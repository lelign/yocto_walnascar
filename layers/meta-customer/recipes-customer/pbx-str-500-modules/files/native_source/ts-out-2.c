...
void dma_timer_callback(unsigned long data)
{
        struct board_info *board = (struct board_info *) data;
...
int tsout_probe(struct platform_device *pdev)
{
        ...
        setup_timer(&board->dma_timer, dma_timer_callback, (unsigned int)board);
        mod_timer(&board->dma_timer, jiffies + msecs_to_jiffies(10));
        return 0;
}

int tsout_remove(struct platform_device *pdev)
{
...
{
        board_info_grob.tsout_class = class_create(THIS_MODULE, DRV_NAME);
